...
import com.wanggaowan.tools.utils.FileUtils
...
fun addGeneratorGFileDependencies(
    ...
    // 目录及文件的创建属于VFS写入，必须在write action中执行
    WriteCommandAction.runWriteCommandAction(project) {
        try {
            var dartTool = pubRoot.root.findChild(".dart_tool")
            if (dartTool == null) {
                dartTool = pubRoot.root.createChildDirectory(module, ".dart_tool")
            }
            var flutterGen = dartTool.findChild("flutter_gen")
            if (flutterGen == null) {
                flutterGen = dartTool.createChildDirectory(module, "flutter_gen")
            }

            val pubspecYaml = flutterGen.findChild("pubspec.yaml")
            if (pubspecYaml == null) {
                val psiFile = flutterGen.createChildData(module, "pubspec.yaml")
                val content = """
                              # Generated by the flutter tool
                              name: synthetic_package
                              description: The Flutter application's synthetic package.
                            """.trimIndent()
                // 此处不能使用PsiElement插入或Document写入，这两种方式写入不能及时写入磁盘
                // 下方执行生成.g.dart指令时，读取不到用这两种方式写入的内容
                FileUtils.write(psiFile.path, content)
            }
        } catch (e: Exception) {
            // 目录创建失败
            e.printStackTrace()
    ...
    // 读取依赖配置需要read action，执行的pub命令耗时较长，不能放在write action中
